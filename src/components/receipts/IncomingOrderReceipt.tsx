...
interface IncomingOrderReceiptProps {
  order: {
    id: string;
    orderNumber: string;
...
export const IncomingOrderReceipt = forwardRef<
  HTMLDivElement,
  IncomingOrderReceiptProps
>(({ order, company }, ref) => {
  ...
  const qrSrc = useQRCode(qrPayload);
    ...
    <div ref={ref} style={containerStyle}>
        ...
        <div style={{ textAlign: "left" }}>
          ...
          {order.supplierInvoice && (
            <div style={{ marginTop: "6px", fontSize: "14px" }}>
              فاتورة المورد: <strong>{order.supplierInvoice}</strong>
            </div>
          )}
        </div>
      </header>
      ...
      >
        <div style={sectionCardStyle}>
          <div style={{ fontSize: "16px", fontWeight: 600 }}>بيانات المورد</div>
          <div>{order.supplier.nameAr}</div>
          {order.supplier.phone && <div>هاتف: {order.supplier.phone}</div>}
          {order.supplier.email && <div>البريد: {order.supplier.email}</div>}
          {order.supplier.address && (
            <div>العنوان: {order.supplier.address}</div>
          )}
        </div>
          ...
          <tbody>
            {order.items.map((item, index) => (
              <tr
                key={
                  item.id ??
                  `${item.item?.id ?? "item"}-${index}-${item.quantity}`
                }
              >
                <td style={{ ...cellStyle, textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#6b7280" }}>
                    ...
                    {item.item?.unit && (
                      <span> | الوحدة: {item.item?.unit}</span>
                    )}
                  </div>
                </td>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "13px", color: "#6b7280" }}>
            تم استلام البضائع وفقاً للتفاصيل أعلاه.
          </div>
        </div>
      </section>
...
IncomingOrderReceipt.displayName = "IncomingOrderReceipt";
